...
class UsersController extends Controller{
	...
	public function postRegister(Request $request){
		...
		if($validator->fails()){
			...
			if(Schools::where('domain', '=', $domain)->first()){
				//判斷信箱和學校編號是否符合
				if((string)Schools::where('domain', '=', $domain)->first()->id === $request->school_id){
					//暫存會員資料到users table內
					$post = Users::create(
					[
						'name' => $request->get('name'),
					...
					//產生信箱驗證碼
					$vcode = Hash::make('vertify:' . $request->username . $request->mail . time());
					//存取現在的user的id
					...
					$result = ['message' => 'success', 'code' => 1, 'data' => ['msg' => '已發送驗證信至 ' . $request->mail]];
					return response()->json($result);
				
				}else{
					
					$result = ['message' => 'failed', 'code' => 0, 'data' => ['msg' => '此信箱非為其學校的網域']];
					return response()->json($result);
				}
				
			}else{
				
				$result = ['message' => 'failed', 'code' => 0, 'data' => ['msg' => '信箱非為已登記學校網域']];
				return response()->json($result);
			}
	...
	public function getConfirm(Request $request){
		...
		if(Vertify_codes::find($request->vcode)->status === 'useful'){
			...
			$result = ['message' => 'success', 'code' => 1, 'data' => ['msg' => '帳號啟用成功']];
			return response()->json($result);

		}else{
			
			$result = ['message' => 'failed', 'code' => 0, 'data' => ['msg' => 'vertify_code unuseful']];
			return response()->json($result);
	...
	public function getLogin(Request $request){
		...
		if ($validator->fails()){
			...
			if(Hash::check($request->get('password'), $password)){
				$token = Hash::make(Users::where('username', '=', $request->get('username'))->first()->username . time());
				...
				$result = ['message' => 'success', 'code' => 1, 'data' => ['token' => $token]];
				return response()->json($result);
			}else{
				
				$result = ['message' => 'failed', 'code' => 0, 'data' => ['msg' => 'password_error']];
				return response()->json($result);
			}
	...
	public function getLogout(Request $request){
		...
		$result = ['message' => 'success', 'code' => 1, 'data' => ['msg' => 'Logout Success!']];
		Session::flush();
		return response()->json($result);
	}
	...
	public function getData(Request $request){
		...
		return response()->json(['message' => 'success', 'code' => 1, 'data' => $result]);
	...
	public function putPassword(Request $request){
		...
		if ($validator->fails()){
			...
			$result = ['message' => 'vali_failed', 'code' => 0, 'data' => $validator->messages()];
			return response()->json($result);
